import { Trigger, triggerBuilder } from './triggers'

export type HTMXSwappingKeys =
  | 'innerHTML'
  | 'outerHTML'
  | 'afterbegin'
  | 'beforebegin'
...
export type HTMXMethodSpreadable = Partial<
  Record<HTMXMethodKeys, Partial<Record<`hx-${HTMXMethodKeys}`, string>>>
>
...
export const resolveHTMXProps = (props: HTMXProps) => {
  ...
  if (replaceUrl)
    build.push({
      'hx-replace-url': typeof replaceUrl === 'boolean' ? booleanToString(replaceUrl) : replaceUrl,
    })
  const r = request || req
